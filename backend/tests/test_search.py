"""Tests unitarios de integración con RediSearch.

Verifican que la búsqueda geográfica usa agregación con distancia calculada y
devuelve modelos de respuesta sin realizar lecturas adicionales por hash.
"""
...
class _FakeRediSearch:
    """Doble mínimo para inspeccionar comandos RediSearch ejecutados."""
...
def test_ft_nearby_rows_uses_aggregate_distance_sorting():
    ...
    assert total == 2
    assert len(rows) == 2
    assert all(isinstance(row, ParkingPlaceNearbyOut) for row in rows)
    assert [row.id for row in rows] == ["aparcamientos:1", "aparcamientos:2"]
    assert [row.distanceMeters for row in rows] == [12.5, 25.0]
    ...
    load_index = aggregate.index("LOAD")

    assert aggregate[load_index + 1] == "*"
