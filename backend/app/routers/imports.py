...
@router.post(
    ...
    description=(
        ...
        "`parking:{id}`.\n\n"
        "Doble buffer sin downtime: la nueva generación se construye bajo "
        "`parking_v2:*` con su propio índice y se hace un swap "
        "(`FT.DROPINDEX` + `UNLINK` + `RENAME` + recreación) al catálogo "
        "activo. La caché de `/parkings/nearby` se invalida incrementando "
        "`cache:version` (O(1), sin `SCAN`). La respuesta incluye totales, "
        "desglose por `sourceDataset` y el nuevo `cache_version`.\n\n"
